    ...
    const navegarRuta = (tab) => {
        ...
        if (tab.tab == 'servicioFinanciero' && tipoConvenio?.convenioRecaudo !== undefined) {
            ...
            if (validarCompletos && redes?.total !== 100 && redes !== undefined) {
                ...
                setMessageModal("Redes requiere configuración para lograr 100%")
        ...
        const rut = (tipoProducto?.[tab.tab] || tipoConvenio?.[tab.tab] && !(tipoProducto?.credito) || tipoConvenio?.[tab.tab] && tipoProducto?.credito && navegarNuevoCredito)||(pathname.split('/'))[3] === 'resultadoSolicitud' && tab.name === 'Resumen Solicitud' ? tab.href : ''
        ...
        <div className="flex" >
            {
                Tabs.map(tab => (
                    ...
                    <button
                        ...
                        className={`w-[20%] py-2 px-4 text-center text-[15px] text-coomeva_color-grisLetras  mx-2 ${pathname === tab.href || activeTab == tab.href ? 'bg-white border-2 border-b-0' : 'bg-coomeva_color-grisPestaña2 '}  rounded-tr-lg rounded-tl-lg`}
                    >
                        {tab.name}
                    </button>
