...
export const fnFilterDataResKnime = async (req) => {
    ...
    try {
        ...
        /* -- */
        if (isNaN(dataReques.convenioGastoOficina) && isNaN(dataReques.convenioGastoPse) && isNaN(dataReques.convenioGastoTrx)) {
            dataReques.convenioGastoCliente = '';
        } else {
            let vConvenioGastoOficina = isNaN(dataReques.convenioGastoOficina) ? 0 : dataReques.convenioGastoOficina;
            let vConvenioGastoPse = isNaN(dataReques.convenioGastoPse) ? 0 : dataReques.convenioGastoPse;
            let vConvenioGastoTrx = isNaN(dataReques.convenioGastoTrx) ? 0 : dataReques.convenioGastoTrx;

            dataReques.convenioGastoCliente = (isNaN(parseInt(vConvenioGastoOficina)) ? 0 : parseInt(vConvenioGastoOficina)) + (isNaN(parseInt(vConvenioGastoPse)) ? 0 : parseInt(vConvenioGastoPse)) + (isNaN(parseInt(vConvenioGastoTrx)) ? 0 : parseInt(vConvenioGastoTrx));
        };
        ...
        if (isNaN(dataReques.convenioMesDiferirOficina) && isNaN(dataReques.convenioMesDiferirPse) && isNaN(dataReques.convenioMesDiferirTrx)) {
            dataReques.convenioMesDiferir = '';
        } else {
            let recaudoOficina = parseInt(isNaN(dataReques.convenioMesDiferirOficina) ? 0 : dataReques.convenioMesDiferirOficina);
            let recaudoPse = parseInt(isNaN(dataReques.convenioMesDiferirPse) ? 0 : dataReques.convenioMesDiferirPse);
            let recaudoTrx = parseInt(isNaN(dataReques.convenioMesDiferirTrx) ? 0 : dataReques.convenioMesDiferirTrx);

            dataReques.convenioMesDiferir = ((recaudoOficina + recaudoPse + recaudoTrx) / (dataReques.convenioRecaudo.gastosPse.length + dataReques.convenioRecaudo.gastoasOficina.length));
        };
