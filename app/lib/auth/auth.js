...
const key = new TextEncoder().encode(process.env.KEY_SECRET);
...
export const getUser = async ({ usuario: foco_user, password: foco_pass }) => {
    ...
    if (resDataProfile.codeState === 200) {
        ...
        try {
            ...
            if (rowsAffected === 2) {
                const perfil1 = rows[0][0]['PERFIL'];
                const perfil2 = rows[0][1]['PERFIL'];
            ...
            if (rowsAffected !== 0) {
                let estadoUser = rows[0][0].ESTADO_USU;

                if (estadoUser !== 1) {
                    USUARIO.status = false;
                    USUARIO.message = 'Usuario inactivo, contacte a su supervisor';
...
export async function adminLogin(prevState, formData) {
    try {
        ...
        console.log(validarSession)

        if (validarSession.sessionActiva === 1) {
            if (usuario.IP === validarSession.ip) {
                const hCookie = hasCookie();
                if (hCookie) {
...
export const obtenerCookieRol = async () => {
    const rol = cookies().get('rol');
    return JSON.stringify(rol.value);
};
