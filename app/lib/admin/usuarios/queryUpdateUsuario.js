...
export const fnQueryUpdateUsuarios = async (req) => {
    ...
    let { usuario, cargo, oficina, regional, canal, aprobador, estado, iObservacion } = Object.fromEntries(req);
    ...
    try {
        ...
        const [rows] = await pool.query(sqlString, [usuario, cargo, oficina, regional, canal, estado, iObservacion, usu]);
        ...
        if (rows.affectedRows === 1) {
            ...
            if (perfiles[0] === '4') {
                ...
                let [insert_rows] = await pool.query(sqlString_insertParametrizador, usuario);
                updateUsuario.insertEnte = insert_rows.affectedRows;
            ...
            if (deleteRows.affectedRows > 0) {
                ...
                for (let i of perfiles) {
                    let [rws] = await pool.query(sqlString_Perfiles, [i, usuario, usu]);
